...
import com.csvreader.CCsvReader;
...
public class UnstructuredStorageReaderUtil {
	...
	public static void readFromStream(InputStream inputStream, String context, Configuration readerSliceConfig,
		...
		try {
			if (null == compress) {
				...
				if ("lzo_deflate".equalsIgnoreCase(compress)) {
					 ...
					 * LZMACompressorInputStream( inputStream); reader = new
					 * BufferedReader(new InputStreamReader(
					 * compressorInputStream, encoding)); }
					 *//*
					 * else if ("pack200".equalsIgnoreCase(compress)) {
					 * CompressorInputStream compressorInputStream = new
					 * Pack200CompressorInputStream( inputStream); reader = new
					 * BufferedReader(new InputStreamReader(
					 * compressorInputStream, encoding)); }
					 *//*
						 * else if ("xz".equalsIgnoreCase(compress)) {
						 * CompressorInputStream compressorInputStream = new
						 * XZCompressorInputStream( inputStream); reader = new
						 * BufferedReader(new InputStreamReader(
						 * compressorInputStream, encoding)); } else if
						 * ("ar".equalsIgnoreCase(compress)) {
						 * ArArchiveInputStream arArchiveInputStream = new
						 * ArArchiveInputStream( inputStream); reader = new
						 * BufferedReader(new InputStreamReader(
						 * arArchiveInputStream, encoding)); } else if
						 * ("arj".equalsIgnoreCase(compress)) {
						 * ArjArchiveInputStream arjArchiveInputStream = new
						 * ArjArchiveInputStream( inputStream); reader = new
						 * BufferedReader(new InputStreamReader(
						 * arjArchiveInputStream, encoding)); } else if
						 * ("cpio".equalsIgnoreCase(compress)) {
						 * CpioArchiveInputStream cpioArchiveInputStream = new
						 * CpioArchiveInputStream( inputStream); reader = new
						 * BufferedReader(new InputStreamReader(
						 * cpioArchiveInputStream, encoding)); } else if
						 * ("dump".equalsIgnoreCase(compress)) {
						 * DumpArchiveInputStream dumpArchiveInputStream = new
						 * DumpArchiveInputStream( inputStream); reader = new
						 * BufferedReader(new InputStreamReader(
						 * dumpArchiveInputStream, encoding)); } else if
						 * ("jar".equalsIgnoreCase(compress)) {
						 * JarArchiveInputStream jarArchiveInputStream = new
						 * JarArchiveInputStream( inputStream); reader = new
						 * BufferedReader(new InputStreamReader(
						 * jarArchiveInputStream, encoding)); } else if
						 * ("tar".equalsIgnoreCase(compress)) {
						 * TarArchiveInputStream tarArchiveInputStream = new
						 * TarArchiveInputStream( inputStream); reader = new
						 * BufferedReader(new InputStreamReader(
			...
			UnstructuredStorageReaderUtil.doReadFromStream(reader, context, readerSliceConfig, recordSender,
					taskPluginCollector);
		} catch (UnsupportedEncodingException uee) {
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.OPEN_FILE_WITH_CHARSET_ERROR,
					String.format("不支持的编码格式 : [%s]", encoding), uee);
		} catch (NullPointerException e) {
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.RUNTIME_EXCEPTION, "运行时错误, 请联系我们",
					e);
		} /*
			 * catch (ArchiveException e) { throw
	...
	public static void doReadFromStream(BufferedReader reader, String context, Configuration readerSliceConfig,
			RecordSender recordSender, TaskPluginCollector taskPluginCollector) {
		String encoding = readerSliceConfig.getString(Key.ENCODING, Constant.DEFAULT_ENCODING);
		/**
		...
		**/
		...
		String fieldDelimiter = readerSliceConfig.getString(Key.FIELD_DELIMITER, 
				Constant.DEFAULT_FIELD_DELIMITER_STR);
		...
		try {
			...
			csvReader = new CCsvReader(reader, fieldDelimiter, column.size());
			/**
			...
			*/
			...
			while ((parseRows = UnstructuredStorageReaderUtil.splitBufferedReader(csvReader)) != null) {
				UnstructuredStorageReaderUtil.transportOneRecord(recordSender, column, parseRows, nullFormat,
						taskPluginCollector);
			}
		} catch (UnsupportedEncodingException uee) {
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.OPEN_FILE_WITH_CHARSET_ERROR,
	...
	public static Record transportOneRecord(RecordSender recordSender, Configuration configuration,
	...
	public static void validateFieldDelimiter(Configuration readerConfiguration) {
		...
		String delimiterInStr = readerConfiguration
				.getString(com.alibaba.datax.plugin.unstructuredstorage.reader.Key.FIELD_DELIMITER, null);
		if (null == delimiterInStr) {
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.REQUIRED_VALUE, String.format(
					"您提供配置文件有误，[%s]是必填参数.", com.alibaba.datax.plugin.unstructuredstorage.reader.Key.FIELD_DELIMITER));
	...
	public static void validateCsvReaderConfig(Configuration readerConfiguration) {
		...
		if (StringUtils.isNotBlank(csvReaderConfig)) {
			try {
				UnstructuredStorageReaderUtil.csvReaderConfigMap = JSON.parseObject(csvReaderConfig,
	...
	public static void setCsvReaderConfig(CsvReader csvReader) {
		if (null != UnstructuredStorageReaderUtil.csvReaderConfigMap
			...
			LOG.info(String.format("CsvReader使用默认值[%s],csvReaderConfig值为[%s]", JSON.toJSONString(csvReader),
					JSON.toJSONString(UnstructuredStorageReaderUtil.csvReaderConfigMap)));
		}
	}
}
