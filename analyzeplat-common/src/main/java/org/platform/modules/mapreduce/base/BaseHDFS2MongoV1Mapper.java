...
public abstract class BaseHDFS2MongoV1Mapper extends Mapper<LongWritable, Text, NullWritable, Text> {
	
	private static final Logger LOG = LoggerFactory.getLogger(BaseHDFS2MongoV1Mapper.class);
	...
	private void bulkInsertDocuments(List<Document> documents) {
        ...
        Set<String> insertDocumentIds = new HashSet<String>();
        Document filter = null;
        Document document = null;
        for (int i = 0, len = documents.size(); i < len; i++) {
        	document = documents.get(i);
        	String _id = document.getString("_id");
        	if (insertDocumentIds.contains(_id)) continue;
        	insertDocumentIds.add(_id);
        	filter = new Document();  
            filter.put("_id", _id);
            FindIterable<Document> docs = collection.find(filter);  
            MongoCursor<Document> iterator = docs.iterator();  
            if (!iterator.hasNext()) insertDocuments.add(document);
        }
