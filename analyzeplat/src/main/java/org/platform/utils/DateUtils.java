...
public class DateUtils {
	...
	public static String getToday() {
		return DateFormatter.DATE.get().format(new Date());
	}

	public static String getYesterday() {
		java.util.Calendar calendar = GregorianCalendar.getInstance();
		calendar.add(Calendar.DATE, -1);
		return DateFormatter.DATE.get().format(calendar.getTime());
	}
	...
	public static boolean runMonthlyJob(String date) {
		...
		if ((null != date) && (!"".equals(date))) {
			try {
				today.setTime(DateFormatter.DATE.get().parse(date));
	...
	public static List<String> getLatestDateList(String date, int n)
		...
		Date toDate = DateFormatter.DATE.get().parse(date);
		...
		for (;;) {
			...
			resultList.add(DateFormatter.DATE.get().format(temp));
		}
		return resultList;
	}
	...
	public static String getTheDayBefore(String date, int n)
			throws ParseException {
		Date inDate = DateFormatter.DATE.get().parse(date);
		java.util.Calendar calendar = GregorianCalendar.getInstance();
	...
	public static List<String> getDateList(String from, String to)
		...
		Date fromDate = DateFormatter.DATE.get().parse(from);
		Date toDate = DateFormatter.DATE.get().parse(to);
		java.util.Calendar gc = GregorianCalendar.getInstance();
		gc.setTime(toDate);
	...
	public static List<String> getMonthDateList(String date)
			throws ParseException {
		Date dateTemp = DateFormatter.DATE.get().parse(date);
		...
		return getDateList(
				DateFormatter.DATE.get().format(from.getTime()),
				DateFormatter.DATE.get().format(to.getTime()));
	}

	public static List<String> getQuarterDateList(String date)
			throws ParseException {
		java.util.Calendar from = GregorianCalendar.getInstance();
		from.setTime(DateFormatter.DATE.get().parse(date));
		java.util.Calendar to = GregorianCalendar.getInstance();
		to.setTime(DateFormatter.DATE.get().parse(date));
	...
	public static long getQuarterBeginTime(String date) throws ParseException {
		java.util.Calendar c = GregorianCalendar.getInstance();
		c.setTime(DateFormatter.DATE.get().parse(date));
