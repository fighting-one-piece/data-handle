...
import org.platform.utils.word.PinYinUtils;

public class ExtractProvinceCityPinYin extends GenericUDF {
		...
		if (StringUtils.isNotBlank(address)) {
			...
			for (int i = 0, len = words.size(); i < len; i++) {
				...
				if (AdministrativeDivisionUtils.getProvinces().contains(word)) {
					...
					result.add(new Text(PinYinUtils.getPinYinFromHanYu(word)));
				}
				if (AdministrativeDivisionUtils.getCities().contains(word)) {
					...
					if (adMatcher.getProvinceMatch() == 0) {
						result.add(new Text(PinYinUtils.getPinYinFromHanYu(AdministrativeDivisionUtils.getCityProvinceMap().get(word))));
					}
					result.add(new Text(PinYinUtils.getPinYinFromHanYu(word)));
				}
			}
