...
public class mergeFile extends Configured implements Tool {
	...
	public static void main(String[] args) {
		try {
			// 输入/输出文件路径
			args = new String[] { "hdfs://192.168.0.10:9000/user/lx/accum/ac",
					"^(00000)+.*$", "hdfs://192.168.0.10:9000/user/lx/accum/as"};
			int exitCode = 0;
			...
			for (int i = 0, len = files.size(); i < len; i++) {
				...
				if ((i % 70== 0 || i == (len - 1)) && i != 0) {
					sb.deleteCharAt(sb.length() - 1);
					String[] str = new String[]{sb.toString(),args[2]+i};
					exitCode = ToolRunner.run(new mergeFile(), str);
	...
	public int run(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		...
		Job job = Job.getInstance(conf, "MergeJob");
	...
	public static class StatisticsFileMap extends Mapper<LongWritable, Text, Text, Text> {
		private Gson gson;
	...
	public static class StatisticsFileReduce extends Reducer<Text, Text, NullWritable, Text> {
