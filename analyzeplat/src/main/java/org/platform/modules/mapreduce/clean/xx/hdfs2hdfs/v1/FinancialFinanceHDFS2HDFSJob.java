...
public class FinancialFinanceHDFS2HDFSJob extends BaseHDFS2HDFSJob {
	@Override
	public Class<? extends BaseHDFS2HDFSV1Mapper> getMapperClass() {
		return FinancialFinaceHDFS2HDFSMapper.class;
	}
	...
	public static void main(String[] args) {
		try {
			int exitCode = 0;
			for (int i = 0; i <= 9; i++) {
				String fss = "hdfs://192.168.0.10:9000/elasticsearch_original/financial/finance/20/records-1-m-0000"
						+ i;
				// String fss =
				// "hdfs://192.168.0.115:9000/elasticsearch/financial/finance/cctest.txt";
				...
				for (Path p : listPath) {
					if (!p.getName().equals("_SUCCESS") && !p.getName().equals("part-r-00000")) {
						args = new String[] { "", fss, "hdfs://192.168.0.10:9000/elasticsearch_original/financial/finance/clean/" + p.getName() };
						exitCode = ToolRunner.run(new FinancialFinanceHDFS2HDFSJob(), args);
					}
				}
			}
...
class FinancialFinaceHDFS2HDFSMapper extends BaseHDFS2HDFSV1Mapper {
	...
	public void handle(Map<String, Object> original, Map<String, Object> correct, Map<String, Object> incorrect) {
		...
		if (original.containsKey("idCard")) {
			...
			if (CleanUtil.matchIdCard(idCard)) {
				...
				flag = CleanTool.cleanT(original, "idCard", patternCard, cardReg, "finance");
			}
		} else {
			flag = CleanTool.cleanS(original, patternCard, cardReg, "idCard", "finance");
		}
		...
		if (original.containsKey("phone")) {
			...
			if (CleanUtil.matchPhone(phone)) {
				...
				tool = CleanTool.cleanT(original, "phone", patternPhone, phoneReg, "finance");
			}
		} else {
			CleanTool.cleanS(original, patternPhone, phoneReg, "phone", "finance");
		}
		// 存在phone但是都不满足
		if (!tool) {
		...
		if (original.containsKey("homeCall")) {
			...
			if (CleanUtil.matchCall(homeCall)) {
				...
				call = CleanTool.cleanT(original, "homeCall", patternCall, callReg, "finance");
			}
		} else {
			CleanTool.cleanS(original, patternCall, callReg, "homeCall", "finance");
		}
		if (!call) {
