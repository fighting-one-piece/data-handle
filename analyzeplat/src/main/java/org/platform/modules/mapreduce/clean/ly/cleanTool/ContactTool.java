	...
	// 需要跳过的特殊字段的集合
			...
			while (newMap.hasNext()) {
				...
				if ("idCard".equals(param)
						|| ("phone".equals(param) && !"idCard".equals(key)
								&& !"homeCall".equals(key))
						|| ("homeCall".equals(param) && !"idCard".equals(key)
								&& !"phone".equals(key))) {
		...
		public static boolean togetherMatch(Map<String,Object> map){
			...
			if((!CleanUtil.matchNum((String)map.get("name"))) && CleanUtil.matchChinese((String)map.get("address"))){
				flag = true;
			}else if((!CleanUtil.matchNum((String)map.get("name"))) && CleanUtil.matchChinese((String)map.get("companyName"))){
				flag = true;
			}
			return flag;
		}
		...
		public static List<String> findOther(String param, String value) {
			...
			if ("homeCall".equals(param) || "phone".equals(param)) {
		...
		public static Map<String,Object> replaceKey(Map<String,Object> map){
			...
			if(map.containsKey("homeCall")){
				map.put("telePhone", map.get("homeCall"));
				map.remove("homeCall");
			}
			if(map.containsKey("companyName")){
				map.put("company", map.get("companyName"));
			...
			if(map.containsKey("remark")){
				map.put("note", map.get("remark"));
				map.remove("remark");
			}
			if(map.containsKey("webSite")){
				map.put("website", map.get("webSite"));
				map.remove("webSite");
			}
			return map;
		}
