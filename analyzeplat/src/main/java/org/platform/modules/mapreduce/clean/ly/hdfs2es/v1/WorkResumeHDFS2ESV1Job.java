...
public class WorkResumeHDFS2ESV1Job extends BaseHDFS2ESV1Job{
		...
		try {
			...
			for (int i = 0, len = filePathList.size(); i < len; i++) {
				...
				if (i>0 && (i % 5 == 0 || i == (len - 1))) {
					...
					String[]  args1= new String[] { "work", "resume", "cisiondata", 
							"192.168.0.10,192.168.0.12","1000", sb.toString(), args[1] + i};
					System.out.println("下标： "+i);
					System.out.println("传入的路径--------->>>"+sb.toString());
					System.out.println("文件输出路径------->>>"+args[1]+i);
					ToolRunner.run(new WorkResumeHDFS2ESV1Job(),args1);
