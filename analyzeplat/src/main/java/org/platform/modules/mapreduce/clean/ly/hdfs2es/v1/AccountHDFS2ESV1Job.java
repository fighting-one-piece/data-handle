...
public class AccountHDFS2ESV1Job extends BaseHDFS2ESV1Job{
			...
			for (int i = 0, len = filePathList.size(); i < len; i++) {
				...
				if (i>0 && (i % 2 == 0 || i == (len - 1))) {
					...
					String[]  args1= new String[] { "account", "account", "cisiondata", 
							"192.168.0.10,192.168.0.12", "10000",sb.toString(),args[1]+i};
					System.out.println("下标： "+i);
					System.out.println("  传入的路径--------->>>"+sb.toString());
					System.out.println("文件输出路径------->>>"+args[1]+i);
					ToolRunner.run(new AccountHDFS2ESV1Job(),args1);
