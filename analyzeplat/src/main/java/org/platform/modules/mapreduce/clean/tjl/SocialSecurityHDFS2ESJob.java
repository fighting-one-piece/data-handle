...
public class SocialSecurityHDFS2ESJob extends BaseHDFS2ESV2Job{
	...
	public static void main(String[] args) {
		...
		for (int j = 1; j <= 1; j++) {
			for (int i = 0; i <= 19; i++) {
				...
				String fss = "hdfs://192.168.0.10:9000/elasticsearch_clean_1/work/socialsecurity/21/records-" + j
						+ "-m-000" + n;// s要小写
				...
				try {
					...
					for (Path p : listPath) {
						if (p.getName().matches(rex)) {
							...
							args = new String[] { "work", "socialsecurity", "cisiondata", "192.168.0.10", p.toString() };
							exitCode = ToolRunner.run(new SocialSecurityHDFS2ESJob(), args);
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
