...
public class MoveHdfsFile {
	// 存放目录下的所有满足条件的绝对路径
	public static List<String> list = new ArrayList<String>();

	public static void main(String[] args) {
		String src = "hdfs://192.168.0.10:9000/user/xx/finance/";
		String dst = "hdfs://192.168.0.10:9000/warehouse_data/financial/finance/";
		/**
		 * src 需要移动的文件或目录 dst 移动的目标目录 param 指定移动文件类型（eg:account email）
		 */
		MoveFileAndReName(src, dst, "finance");
	}
		...
		try {
			...
			if (scan.hasNextLine()) {
				for (int i = 0; i < list.size(); i++) {
					...
					System.out.println(localFileName + " ————>>  " + dst + "/" + param + "__"
							+ uuid.toString().replaceAll("-", "_"));
					fs.rename(new Path(list.get(i)),
							new Path(dst + "/" + param + "__" + uuid.toString().replaceAll("-", "_")));
				}
