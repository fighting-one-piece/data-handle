...
public class ReadDirectory {  
	    		...
	    		try {
	    			if (list[i].isDirectory()) {  
		    			...
		    			if(list[i].getName().matches(regKeyWord)){
		    				File f = new File(list[i].toString());
			    			String l =createPrintStr(list[i].getName()+"  size:  "+(float)getFileSize(f)/1024/1024+"mb", fileLevel);
			    			bk.write(l);
			    			bk.newLine(); 
		    			}else {
		    				File f = new File(list[i].toString());
    ...
    public static void main(String[] args) throws IOException { 
    	 args =new String[]{"D:\\WorkSpace","D:\\out\\a.txt","D:\\out\\b.txt","s"};
    	//保存目录 
		FileWriter fw=new FileWriter(args[1]);  	
		//关键字检索目录
		FileWriter fk = new FileWriter(args[2]); 
