 ...
 *
 */
public class CybercafeHDFS2HDFSJob extends BaseHDFS2HDFSJob {
		...
		try {
			...
			for (int j = 4; j <= 4; j++) {
				for (int i = 0; i <= 9; i++) {
					...
					args = new String[] {
							"hdfs://192.168.0.10:9000/elasticsearch_original/work/cybercafe/20/records-" + j + "-m-000" + n,
							"hdfs://192.168.0.10:9000/elasticsearch_clean_1/work/cybercafe/20/records-" + j + "-m-000"
									+ n };
					System.out.println(args[0] + ":" + args[1]);
					exitCode = ToolRunner.run(new CybercafeHDFS2HDFSJob(), args);
				}
			}
