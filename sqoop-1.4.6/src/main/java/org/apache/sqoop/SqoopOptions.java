...
public class SqoopOptions implements Cloneable {
	...
	/**
	 * Set to true in configuration if you want to put db passwords in the
	 * metastore.
	 */
	...
	private String call;
	private Properties connectionParams; // Properties stored as
											// db.connect.params
	...
	private String tmpDir; // where temp data goes; usually /tmp; not
							// serialized.
	...
	private Class validationThresholdClass; // ValidationThreshold
											// implementation
	private Class validationFailureHandlerClass; // FailureHandler
													// implementation
	...
	private String getStringProperty(Properties props, String propName, String defaultVal) {
	...
	private DelimiterSet getDelimiterProperties(Properties props, String prefix, DelimiterSet defaults) {
		...
		**/
		String field = getStringProperty(props, prefix + ".field", defaults.getFieldsTerminatedBy());
	...
	private void setDelimiterProperties(Properties props, String prefix, DelimiterSet values) {
		...
		*/
		putProperty(props, prefix + ".field", values.getFieldsTerminatedBy());
	...
	/**
	 * Take a comma-delimited list of input and split the elements into an
	 * output array.
	 */
	...
	/**
	 * A put() method for Properties that is tolerent of 'null' values. If a
	 * null value is specified, the property is unset.
	 */
	...
	/**
	 * Given a property prefix that denotes a set of numbered properties, return
	 * an array containing all the properties.
	 *
	 * For instance, if prefix is "foo", then return properties "foo.0",
	 * "foo.1", "foo.2", and so on as an array. If no such properties exist,
	 * return 'defaults'.
	 */
	 ...
	 * @param prefix
	 *            the prefix for qualifying contained property keys.
	 * @param values
	 ...
	 * given prefix. Matching elements from this properties instance are
	 * modified so that their prefix is dropped.
	 * 
	 * @param props
	 *            the properties container
	 * @param prefix
	 *            the prefix qualifying properties that need to be removed
	 * @return a new properties instance that contains all matching elements
	 *         from the container properties.
	 */
	...
	/**
	 * Return a Properties instance that encapsulates all the "sticky" state of
	 * this SqoopOptions that should be written to a metastore to restore the
	 * job later.
	 */
	...
	private void writePasswordProperty(Properties props) {
		...
		if (this.getConf().getBoolean(METASTORE_PASSWORD_KEY, METASTORE_PASSWORD_DEFAULT)) {
			// If the user specifies, we may store the password in the
			// metastore.
	...
	/**
	 * @return the temp directory to use; this is guaranteed to end with the
	 *         file separator character (e.g., '/').
	 */
	...
	/**
	 * Return the name of a directory that does not exist before calling this
	 * method, and does exist afterward. We should be the only client of this
	 * directory. If this directory is not used during the lifetime of the JVM,
	 * schedule it to be removed when the JVM exits.
	 */
	private static String getNonceJarDir(String tmpBase) {
		...
		for (int attempts = 0; attempts < MAX_DIR_CREATE_ATTEMPTS; attempts++) {
			...
			if (hashDir.mkdirs()) {
				...
				// If this directory is not actually filled with files, delete
				// it
				// when the JVM quits.
	 ...
	 * method is intended to be used only by multiple unit tests that want to
	 * isolate themselves from one another. It should not be called during
	 * normal Sqoop execution.
	 */
	...
	private void initDefaults(Configuration baseConfiguration) {
		...
		/**
		this.inputDelimiters = new DelimiterSet(DelimiterSet.NULL_CHAR, DelimiterSet.NULL_CHAR, DelimiterSet.NULL_CHAR,
	 ...
	 *
	 * Normal literal characters return themselves: "x" -&gt; 'x', etc. Strings
	 * containing a '\' followed by one of t, r, n, or b escape to the usual
	 * character as seen in Java: "\n" -&gt; (newline), etc.
	 *
	 * Strings like "\0ooo" return the character specified by the octal sequence
	 * 'ooo'. Strings like "\0xhhh" or "\0Xhhh" return the character specified
	 * by the hex sequence 'hhh'.
	 *
	...
	/**
	 * @return true if underlying statements to be executed in batch mode, or
	 *         false if to be executed in a single multirow statement.
	 */
	...
	/**
	 * @return the user-specified option to modify fields to drop hive
	 *         delimiters
	 */
	...
	/**
	 * @return the user-specified option to specify the replacement string for
	 *         hive delimeters
	 */
	 ...
	 */
	public String getInputFieldDelim() {
		String f = inputDelimiters.getFieldsTerminatedBy();
		return StringUtils.isBlank(f) ? this.outputDelimiters.getFieldsTerminatedBy() : f;
		/**
		 * char f = inputDelimiters.getFieldsTerminatedBy(); if (f ==
		 * DelimiterSet.NULL_CHAR) { return
		 * this.outputDelimiters.getFieldsTerminatedBy(); } else { return f; }
		 **/
	}
	 ...
	 */
	public void setInputFieldsTerminatedBy(String c) {
	 ...
	 *         when parsing. Defaults to false. Set true when
	 *         --input-enclosed-by is used.
	 */
	 ...
	 *         text.
	 */
	public String getOutputFieldDelim() {
	 ...
	 */
	public void setFieldsTerminatedBy(String c) {
	 ...
	 *         EnclosedBy char. default is false; set to true if --enclosed-by
	 *         is used instead of --optionally-enclosed-by.
	 */
	...
	/**
	 * @return the set of delimiters used for parsing the input. This may
	 *         include values implicitly set by the output delimiters.
	 */
	public DelimiterSet getInputDelimiters() {
		return new DelimiterSet(getInputFieldDelim(), getInputRecordDelim(), getInputEnclosedBy(), getInputEscapedBy(),
	...
	/**
	 * @return the name of the compression codec to use when importing. E.g.
	 *         <code>org.apache.hadoop.io.compress.GzipCodec</code>.
	 */
	...
	/*
	 * @return true if the output delimiters have been explicitly set by the
	 * user
	 */
	...
	/**
	 * Flag the output delimiter settings as explicit user settings, or
	 * implicit.
	 */
	...
	/**
	 * @return true if the input delimiters have been explicitly set by the
	 *         user.
	 */
	...
	/**
	 * @return the column which is the key column in a table to be exported in
	 *         update mode.
	 */
	...
	/**
	 * Set "UpdateOnly" to silently ignore new rows during update export. Set
	 * "AllowInsert" to insert new rows during update export.
	 */
	 ...
	 *         generate the DBWritable.write(PreparedStatement) method with
	 *         columns exporting in this order, if it is non-null.
	 */
	...
	/**
	 * Returns true if we should create HBase tables/column families that are
	 * missing.
	 */
	...
	/**
	 * Gets the column to use as the row id in an hbase import. If null, use the
	 * primary key column.
	 */
	...
	/**
	 * Return the name of the column of the import source table to check for
	...
	/**
	 * Set the JobStorage descriptor used to open the saved job this
	 * SqoopOptions belongs to.
	 */
	...
	/**
	 * Get the JobStorage descriptor used to open the saved job this
	 * SqoopOptions belongs to.
	 */
	...
	/**
	 * Set the path name used to do an incremental import of old data which will
	 * be combined with an "new" dataset.
	 */
	...
	/**
	 * Return the path name used to do an incremental import of old data which
	 * will be combined with an "new" dataset.
	 */
	...
	/**
	 * Set the path name used to do an incremental import of new data which will
	 * be combined with an "old" dataset.
	 */
	...
	/**
	 * Return the path name used to do an incremental import of new data which
	 * will be combined with an "old" dataset.
	 */
	...
	/**
	 * Returns true if we should create Accumulo tables/column families that are
	 * missing.
	 */
	...
	/**
	 * Gets the column to use as the row id in an Accumulo import. If null, use
	...
	/**
	 * Gets the visibility token to use. If null, don't assign a visibility.
	 */
