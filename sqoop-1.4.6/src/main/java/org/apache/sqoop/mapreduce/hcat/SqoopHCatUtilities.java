...
public final class SqoopHCatUtilities {
	...
	public static final String HCAT_CLI_MAIN_CLASS = "org.apache.hive.hcatalog.cli.HCatCli";
	...
	/**
	 * A Map using String as key type that ignores case of its key and stores
	...
	/**
	 * A class to hold the instance. For guaranteeing singleton creation using
	 * JMM semantics.
	 */
	...
	public static final Log LOG = LogFactory.getLog(SqoopHCatUtilities.class.getName());
	...
	private void checkHomeDirs(SqoopOptions opts) {
		...
		if (home == null || home.length() == 0) {
			LOG.warn("Hive home is not set. job may fail if needed jar files "
					...
					+ " sqoop-env.sh or provide --hive-home option.  HIVE_HOME defaulted " + " to "
					+ SqoopOptions.getHiveHomeDefault());
		}
		...
		if (home == null || home.length() == 0) {
			LOG.warn("HCatalog home is not set. job may fail if needed jar "
					...
					+ " sqoop-env.sh or provide --hcatalog-home option.  " + "HCAT_HOME defaulted to "
					+ SqoopOptions.getHCatHomeDefault());
		}
	}

	public void configureHCat(final SqoopOptions opts, final Job job, final ConnManager connMgr, final String dbTable,
		...
		LOG.info("HCatalog projected schema fields = " + Arrays.toString(projectedSchema.getFieldNames().toArray()));
	...
	public void validateDynamicPartitionKeysMapping() throws IOException {
		...
		if (missingKeys.length() > 0) {
			throw new IOException("Dynamic partition keys are not "
					+ "present in the database columns.   Missing keys = " + missingKeys.substring(1));
		}
	}
	...
	public void validateHCatTableFieldTypes() throws IOException {
		...
		if (hasComplexFields) {
			...
			throw new IOException("The HCatalog table provided " + getQualifiedHCatTableName()
					+ " has complex field types (" + unsupportedFields + ").  They are currently not supported");
		}
	...
	private void initDBColumnInfo() throws IOException {
		...
		if (dbTableName != null) {
			...
			// Read procedure arguments from metadata
			colInfo.putAll(connManager.getColumnInfoForProcedure(this.options.getCall()));
		} else {
			colInfo.putAll(connManager.getColumnInfoForQuery(options.getSqlQuery()));
		}
		...
		LOG.info("Database column names projected : " + Arrays.toString(dbColumnNames));
	...
	private void createHCatTable() throws IOException {
		StringBuilder sb = new StringBuilder();
		sb.append("create table ").append(escHCatObj(hCatDatabaseName)).append('.');
		...
		for (String col : dbColumnNames) {
			...
			if (type.equals("char") || type.equals("varchar")) {
				...
				if (prec > MAX_HIVE_DECIMAL_PREC) {
					LOG.warn(
							"Truncating precison of column " + col + "  from " + prec + " to " + MAX_HIVE_DECIMAL_PREC);
					prec = MAX_HIVE_DECIMAL_PREC;
				}

				scale = dbColumnInfo.get(col).get(2);
				if (scale > MAX_HIVE_DECIMAL_SCALE) {
					LOG.warn("Truncating precison of column " + col + "  from " + scale + " to "
							+ MAX_HIVE_DECIMAL_SCALE);
	...
	private void validateFieldAndColumnMappings() throws IOException {
		...
		for (Object column : userHiveMapping.keySet()) {
			...
			if (!found) {
				throw new IllegalArgumentException(
						"Column " + column + " not found while mapping database columns to hcatalog columns");
			}
		}
		...
		for (int indx = 0; indx < dbColumnNames.length; ++indx) {
			...
			if (!found) {
				throw new IOException(
						"Database column " + col + " not found in " + "hcatalog table schema or partition schema");
			}
			if (!userMapped) {
				HCatFieldSchema hCatFS = hCatFullTableSchema.get(col);
				if (!hCatFS.getTypeString().equals(hCatColType)) {
					LOG.warn("The HCatalog field " + col + " has type " + hCatFS.getTypeString() + ".  Expected = "
							+ hCatColType + " based on database column type : " + sqlTypeString(colInfo.get(0)));
					LOG.warn("The Sqoop job can fail if types are not " + " assignment compatible");
				}
			}

			if (HiveTypes.isHiveTypeImprovised(colInfo.get(0))) {
				LOG.warn("Column " + col + " had to be cast to a less precise type " + hCatColType + " in hcatalog");
			}
		...
		DefaultStringifier.storeArray(configuration, positions, HCAT_FIELD_POSITIONS_PROP);
	}
	...
	private String getHCatSPFilterStr() {
		if (hCatStaticPartitionKeys != null && hCatStaticPartitionKeys.size() > 0) {
			...
			for (int i = 0; i < hCatStaticPartitionKeys.size(); ++i) {
				...
				filter.append(key).append('=').append('\'').append(val).append('\'');
			}
	...
	private void validateStaticPartitionKey() throws IOException {
		...
		if (hCatStaticPartitionKeys != null && hCatStaticPartitionKeys.size() > 0) {
			for (String key : hCatStaticPartitionKeys) {
				...
				if (!found) {
					throw new IOException("The provided hive partition key " + key + " is not part of the partition "
		...
		configuration.setStrings(HCAT_STATIC_PARTITION_KEY_PROP,
				hCatStaticPartitionKeys.toArray(new String[hCatStaticPartitionKeys.size()]));
	}

	public static void configureImportOutputFormat(SqoopOptions opts, Job job, ConnManager connMgr, String dbTable,
		...
		delimChars = new IntWritable[] {
				 ...
				 */
				new IntWritable(delims.getFieldsTerminatedBy().charAt(0)),
				new IntWritable(delims.getLinesTerminatedBy()), new IntWritable(delims.getEnclosedBy()),
				new IntWritable(delims.getEscapedBy()), new IntWritable(delims.isEncloseRequired() ? 1 : 0), };
		...
		DefaultStringifier.storeArray(config, delimChars, HIVE_DELIMITERS_TO_REPLACE_PROP);
	...
	/**
	 * Resolve a database-specific type to HCat data type. Largely follows
	 * Sqoop's hive translation.
	 * 
	 * @param sqlType
	 *            sql type
	 * @return hcat type
	...
	public void invokeOutputCommitterForLocalMode(Job job) throws IOException {
		if (ConfigurationHelper.isLocalJobTracker(job.getConfiguration()) && isHadoop1()) {
			// HCatalog 0.11- do have special class HCatHadoopShims, however
			// this
			// class got merged into Hive Shim layer in 0.12+. Following method
			// will
