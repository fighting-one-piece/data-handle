...
public class NetezzaExternalTableImportJob extends ImportJobBase {
	...
	public NetezzaExternalTableImportJob(final SqoopOptions opts, ImportJobContext context) {
		super(opts, NetezzaExternalTableTextImportMapper.class, NetezzaExternalTableInputFormat.class,
	...
	protected void propagateOptionsToJob(Job job) {
		...
		if (nullValue != null) {
			conf.set(DirectNetezzaManager.NETEZZA_NULL_VALUE, StringEscapeUtils.unescapeJava(nullValue));
		}
		/**
		conf.setInt(DelimiterSet.OUTPUT_FIELD_DELIM_KEY, options.getOutputFieldDelim());
		**/
		conf.set(DelimiterSet.OUTPUT_FIELD_DELIM_KEY, options.getOutputFieldDelim());
		conf.setInt(DelimiterSet.OUTPUT_RECORD_DELIM_KEY, options.getOutputRecordDelim());
		conf.setInt(DelimiterSet.OUTPUT_ENCLOSED_BY_KEY, options.getOutputEnclosedBy());
		...
		conf.setBoolean(DelimiterSet.OUTPUT_ENCLOSE_REQUIRED_KEY, options.isOutputEncloseRequired());
	...
	protected void configureInputFormat(Job job, String tableName, String tableClassName, String splitByCol)
		...
		DataDrivenDBInputFormat.setInput(job, DBWritable.class, tableName, whereClause, mgr.escapeColName(splitByCol),
				sqlColNames);
