 ...
 */
public class MySQLDumpMapper extends SqoopMapper<String, NullWritable, String, NullWritable> {
	...
	public static class CopyingAsyncSink extends ErrorableAsyncSink {
		...
		protected CopyingAsyncSink(final MySQLDumpMapper.Context context, final PerfCounters ctrs) {
			...
			public void run() {
				...
				try {
					...
					while (true) {
						...
						// this line is of the form "INSERT .. VALUES ( actual
						// value text
						// );" strip the leading preamble up to the '(' and the
						// trailing
						// ');'.
						if (preambleLen == -1) {
							// we haven't determined how long the preamble is.
							// It's constant
						...
						// chop off the leading and trailing text as we write
						// the
						// output to HDFS.
					...
					// flag this error so we get an error status back in the
					// caller.
					setError();
	...
	/**
	 * The ReparsingAsyncSink will instantiate a RecordParser to read
	 * mysqldump's output, and re-emit the text in the user's specified output
	 * format.
	 */
	public static class ReparsingAsyncSink extends ErrorableAsyncSink {
		...
		protected ReparsingAsyncSink(final MySQLDumpMapper.Context c, final Configuration conf,
			...
			public void run() {
				...
				try {
					...
					*/
					...
					String outputFieldDelim = conf.get(MySQLUtils.OUTPUT_FIELD_DELIM_KEY,"");
					...
					char outputEnclose = (char) conf.getInt(MySQLUtils.OUTPUT_ENCLOSED_BY_KEY, DelimiterSet.NULL_CHAR);
					char outputEscape = (char) conf.getInt(MySQLUtils.OUTPUT_ESCAPED_BY_KEY, DelimiterSet.NULL_CHAR);
					...
					while (true) {
						...
						// Wrap the input string in a char buffer that ignores
						// the leading
						// and trailing text.
						...
						// For all of the output fields, emit them using the
						// delimiters
						// the user chooses.
						boolean first = true;
					...
					LOG.error("IOException reading from mysqldump: " + ioe.toString());
					// flag this error so the parent can handle it
					// appropriately.
					setError();
				} catch (InterruptedException ie) {
					LOG.error("InterruptedException reading from mysqldump: " + ie.toString());
	...
	public void map(String splitConditions, NullWritable val, Context context)
		...
		args.add(MySQLUtils.MYSQL_DUMP_CMD); // requires that this is on the
												// path.
		...
		try {
			...
			// Don't use the --where="<whereClause>" version because spaces in
			// it can
			// confuse Java, and adding in surrounding quotes confuses Java as
			// well.
			...
			// Try to wait for stderr to finish, but regard any errors as
			// advisory.
			if (null != errSink) {
				try {
					if (0 != errSink.join()) {
