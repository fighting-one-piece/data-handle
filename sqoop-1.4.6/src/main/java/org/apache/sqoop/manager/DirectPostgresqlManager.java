...
public class DirectPostgresqlManager extends com.cloudera.sqoop.manager.PostgresqlManager {
		...
		private static class PostgresqlStreamThread extends ErrorableThread {
			...
			public void run() {
				...
				try {
					...
					// set the error bit so our caller can see that something
					// went wrong.
					setError();
				} finally {
					if (null != r) {
	...
	/**
	 * @return the Postgresql-specific SQL command to copy the table (
	 *         "COPY .... TO STDOUT").
	 */
	private String getCopyCommand(String tableName) {
		...
		**/
		sb.append(this.options.getOutputFieldDelim());
		sb.append("' CSV ");
	...
	public void importTable(com.cloudera.sqoop.manager.ImportJobContext context) throws IOException, ImportException {
		...
		try {
			// Get the COPY TABLE command to issue, write this to a file, and
			// pass
			// it in to psql with -f filename. Then make sure we delete this
			// file
			...
			// name and the host and port. If the host is localhost and the port
			// is
			// not specified, we don't want to pass this to psql, because we
			// want to
