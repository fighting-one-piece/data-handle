...
public class ClassWriter {
	...
	public static final String PROPERTY_CODEGEN_METHODS_MAXCOLS = "codegen.methods.maxcols";
	 ...
	 * class.
	 *
	 * If the way that we generate classes changes, bump this number. This
	 * number is retrieved by the SqoopRecord.getClassFormatVersion() method.
	 */
	...
	/**
	 * This number confines the number of allowed columns in a single method. It
	 * allows code generation to scale to thousands of columns without running
	 * into "code too large" exceptions.
	 */
	...
	/**
	 * Creates a new ClassWriter to generate an ORM class for a table or
	 * arbitrary query.
	 * 
	 * @param opts
	 ...
	 * @param table
	 *            the name of the table to read. If null, query is taken from
	 *            the SqoopOptions.
	 */
	public ClassWriter(final SqoopOptions opts, final ConnManager connMgr, final String table,
			final CompilationManager compMgr) {
		this.options = opts;
		this.connManager = connMgr;
		this.tableName = table;
		this.compileManager = compMgr;
		this.bigDecimalFormatString = this.options.getConf().getBoolean(ImportJobBase.PROPERTY_BIGDECIMAL_FORMAT,
				ImportJobBase.PROPERTY_BIGDECIMAL_FORMAT_DEFAULT);
		this.maxColumnsPerMethod = this.options.getConf().getInt(PROPERTY_CODEGEN_METHODS_MAXCOLS,
				MAX_COLUMNS_PER_METHOD_DEFAULT);
	}

	/**
	 * Given some character that can't be in an identifier, try to map it to a
	 * string that can.
	 *
	 * @param c
	 *            a character that can't be in a Java identifier
	 * @return a string of characters that can, or null if there's no good
	 *         translation.
	 */
	...
	/**
	 * Coerce a candidate name for an identifier into one which is a valid Java
	 * or Avro identifier.
	 *
	 * Ensures that the returned identifier matches [A-Za-z_][A-Za-z0-9_]* and
	 * is not a reserved word.
	 *
	 * @param candidate
	 *            A string we want to use as an identifier
	 * @return A string naming an identifier which compiles and is similar to
	 *         the candidate.
	 */
	public static String toIdentifier(String candidate) {
		...
		for (char c : candidate.toCharArray()) {
			if (Character.isJavaIdentifierStart(c) && first) {
				...
				// If it's just not allowed to be the first char, add a leading
				// '_'.
				// If we have a reasonable translation (e.g., '-' -> '_'), do
				// that.
				// Otherwise, drop it.
	...
	/**
	 * Coerce a candidate name for an identifier into one which will definitely
	 * compile.
	 *
	 * Ensures that the returned identifier matches [A-Za-z_][A-Za-z0-9_]* and
	 * is not a reserved word.
	...
	public static String toJavaIdentifier(String candidate) {
		...
		if (isReservedWord(output)) {
			...
			/*
			 * We're using candidate.startsWith instead of output.startsWith on
			 * purpose to preserve backward compatibility as much as possible.
			 * For example column "9isLegalInSql" was translated into
			 * "_9isLegalInSql" in original code and will translate to same
			 * "_9isLegalInSql" now. However it would be translated to
			 * "__9isLegalInSql" (notice that there are two "_" at the begging)
			 * if we would use output.startsWith instead.
			 */
	...
	private String dbSetterForType(String javaType) {
		...
		// See dbGetterForType() for the logic used here; it's basically the
		// same.
	...
	private String stringifierForType(String javaType, String colName) {
		if (javaType.equals("String")) {
			// Check if it is null, and write the null representation in such
			// case
			String r = colName + "==null?\"" + this.options.getNullStringValue() + "\":" + colName;
			return r;
		} else if (javaType.equals("java.math.BigDecimal") && this.bigDecimalFormatString) {
			...
			// This is an object type -- just call its toString() in a null-safe
			// way.
			// Also check if it is null, and instead write the null
			// representation
			// in such case
			String r = colName + "==null?\"" + this.options.getNullNonStringValue() + "\":" + "\"\" + " + colName;
			return r;
	...
	private String rpcGetterForType(String javaType, String inputObj, String colName) {
		if (javaType.equals("Integer")) {
			...
			return "    this." + colName + " = new Timestamp(" + inputObj + ".readLong());\n" + "    this." + colName
					+ ".setNanos(" + inputObj + ".readInt());\n";
		} else if (javaType.equals("java.math.BigDecimal")) {
			return "    this." + colName + " = " + BigDecimalSerializer.class.getCanonicalName() + ".readFields("
					+ inputObj + ");\n";
		} else if (javaType.equals(ClobRef.class.getName())) {
			return "    this." + colName + " = " + LobSerializer.class.getCanonicalName() + ".readClobFields("
					+ inputObj + ");\n";
		} else if (javaType.equals(BlobRef.class.getName())) {
			return "    this." + colName + " = " + LobSerializer.class.getCanonicalName() + ".readBlobFields("
					+ inputObj + ");\n";
		} else if (javaType.equals(BytesWritable.class.getName())) {
			return "    this." + colName + " = new BytesWritable();\n" + "    this." + colName + ".readFields("
					+ inputObj + ");\n";
		} else {
			LOG.error("No ResultSet method for Java type " + javaType);
			return null;
	 ...
	 * @param inputObj
	 *            name of the DataInput to read from
	 * @param colName
	 *            the column name to read.
	 * @return
	 */
	 ...
	 *            the type to write
	 * @param outputObj
	 *            the name of the DataOutput to write to
	 * @param colName
	...
	private String rpcSetterForType(String javaType, String outputObj, String colName) {
		if (javaType.equals("Integer")) {
			...
			return "    " + outputObj + ".writeLong(this." + colName + ".getTime());\n" + "    " + outputObj
					+ ".writeInt(this." + colName + ".getNanos());\n";
		} else if (javaType.equals(BytesWritable.class.getName())) {
	...
	/**
	 * Get the number of methods that should be generated for a particular
	 * column set.
	 * 
	 * @param colNames
	 * @param size
	...
	/**
	 * Get the top boundary when iterating through columns on a per method
	 * basis.
	 * 
	 * @param colNames
	 * @param methodNumber
	 ...
	 * @param sb
	 *            - StringBuilder to append code to
	 */
	private void generateFields(Map<String, Integer> columnTypes, String[] colNames, String className,
			StringBuilder sb) {
	 ...
	 *            - StringBuilder to append code to
	 */
	private void generateEquals(Map<String, Integer> columnTypes, String[] colNames, String className,
			StringBuilder sb) {
		...
		if (numberOfMethods > 1) {
			...
			myGenerateEquals(columnTypes, colNames, className, sb, 0, maxColumnsPerMethod, false);
		}
		sb.append("  }\n");

		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateEquals(columnTypes, colNames, className, sb, i, maxColumnsPerMethod, true);
		}
	}
	 ...
	 * @param wrapInMethod
	 *            - wrap body in a method.
	 */
	private void myGenerateEquals(Map<String, Integer> columnTypes, String[] colNames, String className,
			StringBuilder sb, int methodNumber, int size, boolean wrapInMethod) {
		...
		for (int i = size * methodNumber; i < topBoundary(colNames, methodNumber, size); ++i) {
			...
			sb.append("    equal = equal && (this." + col + " == null ? that." + col + " == null : this." + col
					+ ".equals(that." + col + "));\n");
		}
		sb.append("    return equal;\n");
	...
	private void generateDbRead(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb) {
		...
		if (numberOfMethods > 1) {
			...
			myGenerateDbRead(columnTypes, colNames, sb, 0, maxColumnsPerMethod, false);
		}
		sb.append("  }\n");

		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateDbRead(columnTypes, colNames, sb, i, maxColumnsPerMethod, true);
		}
	}
	 ...
	 *            - wrap body in a method.
	 */
	private void myGenerateDbRead(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
		...
		for (int i = methodNumber * size; i < topBoundary(colNames, methodNumber, size); ++i) {
			...
			String javaType = toJavaType(col, sqlType);
			if (null == javaType) {
				LOG.error("No Java type for SQL type " + sqlType + " for column " + col);
				continue;
			}
			...
			sb.append(
					"    this." + col + " = JdbcWritableBridge." + getterMethod + "(" + (i + 1) + ", __dbResults);\n");
		}
	...
	private void generateLoadLargeObjects(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb) {
		...
		if (numberOfMethods > 1) {
			...
			myGenerateLoadLargeObjects(columnTypes, colNames, sb, 0, maxColumnsPerMethod, false);
		}

		sb.append("  }\n");

		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateLoadLargeObjects(columnTypes, colNames, sb, i, maxColumnsPerMethod, true);
		}
	}
	 ...
	 * delayed objects (that require the Context from the mapper).
	 */
	private void myGenerateLoadLargeObjects(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
			int methodNumber, int size, boolean wrapInMethod) {
		...
		for (int i = methodNumber * size; i < topBoundary(colNames, methodNumber, size); ++i) {
			...
			String getterMethod = dbGetterForType(javaType);
			if ("readClobRef".equals(getterMethod) || "readBlobRef".equals(getterMethod)) {
				// This field is a blob/clob field with delayed loading. Call
				// the
				// appropriate LargeObjectLoader method (which has the same name
				// as a
				// JdbcWritableBridge method).
				sb.append("    this." + col + " = __loader." + getterMethod + "(" + (i + 1)
						+ ", this.__cur_result_set);\n");
			}
		}
	...
	private void generateDbWrite(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb) {
		...
		if (numberOfMethods > 1) {
			...
			myGenerateDbWrite(columnTypes, colNames, sb, 0, maxColumnsPerMethod, false);
		}
		...
		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateDbWrite(columnTypes, colNames, sb, i, maxColumnsPerMethod, true);
		}
	}
	 ...
	 *            - wrap body in a method.
	 */
	private void myGenerateDbWrite(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
			int methodNumber, int size, boolean wrapInMethod) {
	...
	private void generateHadoopRead(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb) {
		...
		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateHadoopRead(columnTypes, colNames, sb, i, maxColumnsPerMethod, true);
		}
	}
	 ...
	 *            - wrap body in a method.
	 */
	private void myGenerateHadoopRead(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
			int methodNumber, int size, boolean wrapInMethod) {
		if (wrapInMethod) {
			sb.append("  public void readFields" + methodNumber + "(DataInput __dataIn) " + "throws IOException {\n");
		}
	...
	private void generateCloneMethod(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb) {
		...
		if (numberOfMethods > 1) {
			...
			myGenerateCloneMethod(columnTypes, colNames, sb, 0, maxColumnsPerMethod, false);
		}
		...
		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateCloneMethod(columnTypes, colNames, sb, i, maxColumnsPerMethod, true);
		}
	}
	 ...
	 *            - wrap body in a method.
	 */
	private void myGenerateCloneMethod(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
			int methodNumber, int size, boolean wrapInMethod) {
		TableClassName tableNameInfo = new TableClassName(options);
		String className = tableNameInfo.getShortClassForTable(tableName);

		if (wrapInMethod) {
			sb.append("  public void clone" + methodNumber + "(" + className
		...
		for (int i = methodNumber * size; i < topBoundary(colNames, methodNumber, size); ++i) {
			...
			if (null == javaType) {
				continue;
			} else if (javaType.equals("java.sql.Date") || javaType.equals("java.sql.Time")
					|| javaType.equals("java.sql.Timestamp") || javaType.equals(ClobRef.class.getName())
					|| javaType.equals(BlobRef.class.getName())) {
				sb.append("    o." + colName + " = (o." + colName + " != null) ? (" + javaType + ") o." + colName
						+ ".clone() : null;\n");
			} else if (javaType.equals(BytesWritable.class.getName())) {
				sb.append("    o." + colName + " = (o." + colName + " != null) ? " + "new BytesWritable(Arrays.copyOf("
						+ colName + ".getBytes(), " + colName + ".getLength())) : null;\n");
			}
		}
	 ...
	 *            - number of columns per method
	 */
	private void myGenerateSetField(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
	...
	private void generateGetFieldMap(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb) {
		...
		if (numberOfMethods > 1) {
			...
			myGenerateGetFieldMap(columnTypes, colNames, sb, 0, maxColumnsPerMethod, false);
		}
		sb.append("    return __sqoop$field_map;\n");
		sb.append("  }\n\n");

		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateGetFieldMap(columnTypes, colNames, sb, i, maxColumnsPerMethod, true);
		}
	}
	 ...
	 *            - wrap body in a method.
	 */
	private void myGenerateGetFieldMap(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
			int methodNumber, int size, boolean wrapInMethod) {
		if (wrapInMethod) {
	...
	private void generateToString(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb) {
		...
		// This variant allows the user to specify whether or not an
		// end-of-record
		...
		// This toString() variant allows the user to specify delimiters, as
		// well
		...
		sb.append("    String fieldDelim = delimiters.getFieldsTerminatedBy();\n");
		...
		if (numberOfMethods > 1) {
			...
			myGenerateToString(columnTypes, colNames, sb, true, 0, maxColumnsPerMethod, false);
		}
		...
		boolean first = true;
		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateToString(columnTypes, colNames, sb, first, i, maxColumnsPerMethod, true);
			first = false;
		}
	}
	 ...
	 *            - wrap body in a method.
	 */
	private void myGenerateToString(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
			boolean first, int methodNumber, int size, boolean wrapInMethod) {
		// This toString() variant allows the user to specify delimiters, as
		// well
		// as whether or not the end-of-record delimiter should be added to the
		// string. Use 'false' to do reasonable things with TextOutputFormat,
		...
		if (wrapInMethod) {
			...
			sb.append("StringBuilder __sb, String fieldDelim) {\n");
		}
		...
		for (int i = methodNumber * size; i < topBoundary(colNames, methodNumber, size); ++i) {
			...
			if (javaType.equals("String") && options.doHiveDropDelims()) {
				...
				sb.append("    __sb.append(FieldFormatter.hiveStringReplaceDelims(" + stringExpr + ", \""
						+ options.getHiveDelimsReplacement() + "\", " + "delimiters));\n");
			} else {
	...
	private void parseColumn(String colName, int colType, StringBuilder sb) {
		...
		if (javaType.equals("String")) {
			// TODO(aaron): Distinguish between 'null' and null. Currently they
			// both
			...
			// We cannot use Byte.parse() which always assumes a signed decimal
			// byte
	...
	private void generateParser(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb) {
		...
		// Embed into the class the delimiter characters to use when parsing
		// input
		...
		// The parser object which will do the heavy lifting for field
		// splitting.
		...
		if (numberOfMethods > 1) {
			...
			myGenerateParser(columnTypes, colNames, sb, 0, maxColumnsPerMethod, false);
		}
		sb.append("  }\n\n");

		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateParser(columnTypes, colNames, sb, i, maxColumnsPerMethod, true);
		}
	}
	 ...
	 *            - wrap body in a method.
	 */
	private void myGenerateParser(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
			int methodNumber, int size, boolean wrapInMethod) {
		// The wrapper methods call __loadFromFields() to actually interpret the
		// raw field data as string, int, boolean, etc. The generation of this
		// method is type-dependent for the fields.
		...
		sb.append("    throw new RuntimeException(" + "\"Can't parse input data: '\" + __cur_str + \"'\", e);");
		sb.append("    }");
		if (wrapInMethod) {
			sb.append("  }\n\n");
		}
	...
	private void generateHadoopWrite(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb) {
		...
		if (numberOfMethods > 1) {
			...
			myGenerateHadoopWrite(columnTypes, colNames, sb, 0, maxColumnsPerMethod, false);
		}

		sb.append("  }\n");

		for (int i = 0; i < numberOfMethods; ++i) {
			myGenerateHadoopWrite(columnTypes, colNames, sb, i, maxColumnsPerMethod, true);
		}
	}
	 ...
	 *            - wrap body in a method.
	 */
	private void myGenerateHadoopWrite(Map<String, Integer> columnTypes, String[] colNames, StringBuilder sb,
			int methodNumber, int size, boolean wrapInMethod) {
		if (wrapInMethod) {
			sb.append("  public void write" + methodNumber + "(DataOutput __dataOut) " + "throws IOException {\n");
		}
	...
	/**
	 * Create a list of identifiers to use based on the true column names of the
	 * table.
	 * 
	 * @param colNames
	 *            the actual column names of the table.
	 * @return a list of column names in the same order which are cleaned up to
	 *         be used as identifiers in the generated Java class.
	 */
	...
	private void logORMSelfGenerationMessage() {
		LOG.info("The connection manager declares that it self manages mapping"
				+ " between records & fields and rows & columns.  No class will" + " will be generated.");
	}
	...
	public void generate() throws IOException {
		...
		if (colNames == null || colNames.length == 0) {
			throw new IllegalArgumentException("There is no column found in the " + "target table " + tableName
		...
		for (int i = 0; i < colNames.length; i++) {
			...
			if (identifier.isEmpty()) { // Name can't be blank
				throw new IllegalArgumentException(
						"We found column without column " + "name. Please verify that you've entered all column names "
			...
			// Make sure the col->type mapping holds for the new identifier
			// name, too
			String col = colNames[i];
			Integer type = columnTypes.get(col);
			if (type == null) {
				// column doesn't have a type, means that is illegal column
				// name!
			...
			for (Object column : mapping.keySet()) {
				if (!uniqColNames.contains((String) column)) {
					throw new IllegalArgumentException("No column by the name " + column
							+ "found while importing data; expecting one of " + uniqColNames);
				}
			}
		}
	...
	protected String[] getColumnNames(Map<String, Integer> columnTypes) {
		...
		if (null == colNames) {
			if (null != tableName) {
				...
				colNames = connManager.getColumnNamesForProcedure(this.options.getCall());
			} else {
	 ...
	 * @param colNames
	 *            - ordered list of column names for table.
	 * @param dbWriteColNames
	 *            - ordered list of column names for the db write() method of
	 *            the class.
	 * @return - A StringBuilder that contains the text of the class code.
	 */
	private StringBuilder generateClassForColumns(Map<String, Integer> columnTypes, String[] colNames,
			String[] dbWriteColNames) {
		...
		// TODO(aaron): Generate hashCode(), compareTo(), equals() so it can be
		// a
		// WritableComparable
